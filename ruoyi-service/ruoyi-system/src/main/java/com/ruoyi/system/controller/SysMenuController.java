...
public class SysMenuController extends BaseController
{
    ...
    public SysMenu get(@PathVariable("menuId") String menuId)
    {
        return sysMenuService.selectMenuById(menuId);
    }

    @GetMapping("perms/{userId}")
    public Set<String> perms(@PathVariable("userId") String userId)
    {
    ...
    public List<SysMenu> role(@PathVariable("roleId") String roleId)
    {
    ...
    public R addSave(@RequestBody SysMenu sysMenu)
    {
        ...
        if (UserConstants.MENU_NAME_NOT_UNIQUE.equals(sysMenuService.checkMenuNameUnique(sysMenu))) {
            return R.error("该菜单名称已存在");
        }
